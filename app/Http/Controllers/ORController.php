...
//            Storage::put('txtForWall/'.$shop.'/newStickers'.'/item'.$j.'.txt',$j.','.$j.','.$uploadNames[$i].','.$uploadNames[$i].'.'.$uploadfiles[$i]->extension());
    ...
    public function getStickerRanking($startDate,$endDate){
        ...
        $query="SELECT number,sum(quantity),DENSE_RANK() OVER(ORDER BY sum(quantity) DESC) AS ranking FROM order_items INNER JOIN order_sheet ON order_sheet.code = order_items.code_sales INNER JOIN items ON order_items.code_product = items.code WHERE date > '".$startDate."' AND date < '".$endDate."' GROUP BY number ORDER BY sum(quantity);";
